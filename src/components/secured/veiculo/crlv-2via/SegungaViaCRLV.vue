...
//                return vinculos;
        ...
        methods: {
            ...
            goTermo: function(vinculo) {
                crlv2ViaStore.commit('setChassi', {chassi: vinculo.chassi, reemitir: false});
                this.$router.push({path: '/area-segura/segunda-via-crlv/termo'});
            },
            reemitirBoleto: function(solicitacao) {
                crlv2ViaStore.commit('setChassi', {chassi: solicitacao.chassi, reemitir: true});
        ...
        created: function () {
            new VeiculoService(this.$http).findSolicitacoesCrlv().then(
                response => {
